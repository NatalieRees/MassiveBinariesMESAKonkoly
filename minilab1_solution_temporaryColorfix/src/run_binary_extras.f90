...
! *********************************************************************** 
      ...
      subroutine data_for_extra_binary_history_columns(binary_id, n, names, vals, ierr)
         use colors_lib, only : get_abs_bolometric_mag
         type (binary_info), pointer :: b
         integer, intent(in) :: binary_id
         integer, intent(in) :: n
         character (len=maxlen_binary_history_column_name) :: names(n)
         real(dp) :: vals(n)
         real(dp) :: abs_mag_F450W_1, abs_mag_F450W_2, BC_primary, BC_secondary, abs_mag_1, abs_mag_2
         ...
         ! primary
         ! write(*,*) 'Primary Teff, logg, m_div_h, logL = ', b% s1% Teff, b% s1% photosphere_logg,  m_div_h,  b% s1% photosphere_L
         ! abs_mag_V_1 = get_abs_mag_by_name(mag_band_name, safe_log10(b% s1% Teff), &
         !       b% s1% photosphere_logg, m_div_h, b% s1% photosphere_L, ierr)

         abs_mag_1 = get_abs_bolometric_mag(b% s1% photosphere_L)
         abs_mag_2 = get_abs_bolometric_mag(b% s2% photosphere_L)

         BC_primary = -2.296 * pow4(safe_log10(b% s1% Teff)) + 45.584 * pow3(safe_log10(b% s1% Teff)) - 337.826 * pow2(safe_log10(b% s1% Teff)) + 1100.973 * safe_log10(b% s1% Teff) -1328.853
         BC_secondary = -2.232 * pow4(safe_log10(b% s2% Teff)) + 44.331 * pow3(safe_log10(b% s2% Teff)) -328.700 * pow2(safe_log10(b% s2% Teff)) + 1071.741 * safe_log10(b% s2% Teff) -1294.131

         abs_mag_F450W_1 = abs_mag_1 - BC_primary
         abs_mag_F450W_2 = abs_mag_2 - BC_secondary


         ! BC_primary = get_bc_by_name(mag_band_name, safe_log10(b% s1% Teff), &
         !       b% s1% photosphere_logg, m_div_h, ierr)
         write(*,*) 'Primary BC = ', BC_primary
         write(*,*) 'Secondary BC = ', BC_secondary
         ! ! get_bc_id_by_name

         ! ! secondary
         ! write(*,*) 'Secondary Teff, logg, m_div_h, logL = ', b% s2% Teff, b% s2% photosphere_logg,  m_div_h,  b% s2% photosphere_L
         ...
         write(*,*) 'primary V band mag = ', abs_mag_F450W_1
         write(*,*) 'secondary V band mag = ', abs_mag_F450W_2

         names(2) = 'F450W_primary'
         vals(2) = abs_mag_F450W_1

         names(3) = 'F450W_secondary'
         vals(3) = abs_mag_F450W_2

         b% xtra(3) = abs_mag_F450W_1
         b% xtra(4) = abs_mag_F450W_2
...
!        b% s1% job% warn_run_star_extras = .false.
      ...
      integer function extras_binary_start_step(binary_id,ierr)
         ...
         ! call colors_init(b% s1% job% color_num_files,b% s1% job% color_file_names,b% s1% job% color_num_colors,ierr)
      
      end function  extras_binary_start_step
      
      !Return either keep_going, retry or terminate
      integer function extras_binary_check_model(binary_id)
         use chem_lib, only : chem_M_div_h ! not needed
         ...
         ! abs_mag_V_1 = get_bc_by_name(mag_band_name, safe_log10(b% s1% Teff), &
         !       b% s1% photosphere_logg, m_div_h, ierr)
         ! write(*,*) 'Primary BC = ', abs_mag_V_1
         ! ! get_bc_id_by_name
